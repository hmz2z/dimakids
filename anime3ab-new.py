...
def scrape_anime_page(anime_data):
    ...
    download_plan = []
    with console.status("[bold bright_green]Finding all available links...") as st:
        for num in selected_numbers:
            ep_data = next((ep for ep in episodes_list if ep["num"] == num), None)
            if ep_data:
                st.update(f"Fetching links for Episode {num}...")
    ...
    summary_table.add_column("Ep", style="bright_cyan")
    summary_table.add_column("1080p", style="bright_green")
    summary_table.add_column("720p", style="bright_blue")
    summary_table.add_column("480p", style="dim")
    for item in download_plan:
        summary_table.add_row(
            f"{item['num']:02d}",
            "✓" if "1080p" in item["links"] else "✗",
            "✓" if "720p" in item["links"] else "✗",
            "✓" if "480p" in item["links"] else "✗",
        )
    ...
    for item in download_plan:
        ...
        for quality in QUALITY_PRIORITY:
            if quality in item["links"]:
                episode_links[quality] = item["links"][quality]
        if episode_links:
            all_resolved_links["episodes"][str(item["num"])] = episode_links
    ...
    for ep_num_str, links_dict in all_resolved_links["episodes"].items():
        ...
        for quality in QUALITY_PRIORITY:
            if quality in links_dict:
                ...
                break
        if best_url:
            final_download_list.append({"num": ep_num, "url": best_url, "quality": best_quality})

    if not final_download_list:
        console.print("[bold red]Could not resolve any direct download links.[/bold red]")
        return
        ...
        while not download_successful and attempt < max_retries:
            ...
            try:
                if attempt > 1:
                    ...
                    if ep_data:
                        new_links = get_episode_download_links(ep_data["link"])
                        if item["quality"] in new_links:
                            current_url = new_links[item["quality"]]
                        else:
                            for q in QUALITY_PRIORITY:
                                if q in new_links:
                                    current_url = new_links[q]
                                    item["quality"] = q
                                    break
                    time.sleep(3)
                ...
                ep_data = next((ep for ep in episodes_list if ep["num"] == item["num"]), None)
                referer = ep_data["link"] if ep_data else ""
                download_successful = download_file(current_url, filepath, filename, referer)
    ...
    if downloaded_eps:
        console.print(f"[bright_green]✅ Successfully downloaded episodes: {', '.join(map(str, sorted(downloaded_eps)))}[/bright_green]")
    if failed_eps:
        console.print(f"[red]❌ Failed to download episodes: {', '.join(map(str, sorted(failed_eps)))}[/red]")
